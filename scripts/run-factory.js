const main = async () => {
    ...
    await createPoapNft(factoryContract, nftIds[0]);
};
...
const getCreatorNFTs = async (factoryContract) => {
    ...
    const nfts = await factoryContract.getAddressCreatorNFTs(
        signers[0].address
    );
...
const createPoapNft = async (factoryContract, id) => {
    let txn = await factoryContract.makeAPoapNFT(id);
    await txn.wait();
};
